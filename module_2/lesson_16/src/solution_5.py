def spiral_matrix(width: int) -> list:
    matrix: list[list[int]] = [[0 for j in range(width)] for i in range(width)] 
    values: list[list[int]] = [[0, 1], [1, 0], [0, -1], [-1, 0]]
    x = y = current = 0
    
    for i in range(width * width):
        if (    x + values[current][0] == width 
                or y + values[current][1] == width 
                or matrix[x + values[current][0]][y + values[current][1]]):
